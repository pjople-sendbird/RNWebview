...
import { AudioInputDeviceInfo, VideoInputDeviceInfo } from '.';
...
const SbCallsProvider = ({
    ...
    const init = useCallback<ContextType['init']>(nAppId => {
        ...
        // warodri SendbirdCall.updateMediaDevices({ audio: true, video: true });
    }, []);
    ...
    const updateMediaDevices = useCallback<ContextType['updateMediaDevices']>(
        (constraints) => {
            // warodri SendbirdCall.updateMediaDevices(constraints);
        }, []
    );
